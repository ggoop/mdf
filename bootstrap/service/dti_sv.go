...
func (s *DtiSv) RunRemotes(entID string, dtiIDs []string, params map[string]string, ctx context.Context) error {
	dtiRemotes := make([]model.DtiRemote, 0)
	...
	for _, k := range keys {
		if m, ok := groups[k]; ok && len(m) > 0 {
			...
			for i, _ := range m {
				n.Add(1)
				go func(entID string, dtiRemote model.DtiRemote, params map[string]string, ctx context.Context) {
...
func (s *DtiSv) runRemote(entID string, dtiRemote model.DtiRemote, params map[string]string, ctx context.Context) error {
	defer func() {
		if r := recover(); r != nil {
	...
	if dtiRemote.Node.Host != "" && strings.Index(strings.ToLower(dtiRemote.Node.Host), "http") == 0 {
		...
		if u, _ := reg.GetServerAddr(dtiRemote.Node.Host); u != "" {
			remoteUrl, _ = url.Parse(u)
		}
	}
	if dtiRemote.Path != "" {
...
func (s *DtiSv) run_handResult(entID string, dtiRemote model.DtiRemote, body []byte, params map[string]string, ctx context.Context) error {
	var remoteUrl *url.URL
	if dtiRemote.Local.Host != "" && strings.Index(strings.ToLower(dtiRemote.Local.Host), "http") == 0 {
		...
		if u, _ := reg.GetServerAddr(dtiRemote.Local.Host); u != "" {
			remoteUrl, _ = url.Parse(u)
		}
	}
	if remoteUrl.Host == "" {
