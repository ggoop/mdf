...
func (c *DtiHandProc) getDSN() (driverName, dataSourceName string, err error) {
	...
	if c.Group == "dti" {
		configs.Default.UnmarshalValue("dti", &dbConfig)
	} else if c.Group == "localhost" || c.Group == "db" {
		configs.Default.UnmarshalValue("db", &dbConfig)
	} else {
		configs.Default.UnmarshalValue("dti."+c.Group, &dbConfig)
	}
...
func (c *DtiHandProc) Do() {
	...
	if err := c.Ctx.ReadJSON(&otherParams); err != nil {
		glog.Error(err)
	}
	...
	otherParams = make(map[string]interface{})
	if err := c.Ctx.ReadForm(&otherParams); err != nil {
		glog.Error(err)
	}
	if len(otherParams) > 0 {
		for k, v := range otherParams {
	...
	rtn := results.Map{"data": nil, "path": c.Path, "name": c.Ctx.GetHeader("REMOTE_CODE")}
...
func (c *DtiHandProc) toError(data interface{}, code ...int) {
	...
	c.Ctx.StatusCode(stCode)
	c.Ctx.ContentType(context.ContentJSONHeaderValue)
	resultByte, err := json.Marshal(obj)
	if err != nil {
		c.Ctx.Write([]byte(err.Error()))
	} else {
		c.Ctx.Write(resultByte)
	}
}
