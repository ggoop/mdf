...
func (s *MdSv) TakeDataByQ(token *utils.TokenContext, req *md.ReqContext) (map[string]interface{}, error) {
	...
	if entField != nil && entField.Code != "" {
		exector.Where(fmt.Sprintf("%s=?", entField.DbName), token.EntID())
	}
...
//执行命令
func (s *MdSv) DoAction(token *utils.TokenContext, req *md.ReqContext) *md.ResContext {
	return md.DoAction(token, req)
}
