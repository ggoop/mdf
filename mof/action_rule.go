...
func DoAction(req ReqContext) (interface{}, error) {
	if req.MainEntity == "" && req.PageID != "" {
		pageMD := md.MDWidget{}
		if err := repositories.Default().Model(pageMD).Where("page_id=?", req.PageID).Take(&pageMD).Error; err != nil {
			return nil, err
		}
		req.MainEntity = pageMD.DsEntry
	}
	res := &ResContext{}
	...
	if req.Rule != "" {
		...
		//查询页面命令
		repositories.Default().Where("owner_id=? and code=? and type=?", req.PageID, req.Command, "action").Take(&command)
		if command.ID == "" { //查找实体命令
			repositories.Default().Where("owner_id=? and code=? and type=?", req.MainEntity, req.Command, "entity").Take(&command)
		}
